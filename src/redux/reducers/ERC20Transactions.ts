import {
  ...
  SetContractAddressPayload,
  SetERC20AllowancePayload,
  SetERC20BalancePayload,
  SetTokenNamePayload,
  ...
  switch (action.type) {
    ...
    case CREATE_CONTRACT_INSTANCE: {
      return ((action: CreateContractInstancePayload) => {
        ...
        //   store in state
        return { ...state, contractInstance: tokenContractInstance };
      })(action);
    }
    ...
    case SET_TOKEN_BALANCE: {
      return ((action: SetERC20BalancePayload) => ({ ...state, balance: action.balance }))(action);
    }

    case SET_TOKEN_NAME: {
      return ((action: SetTokenNamePayload) => ({ ...state, name: action.name }))(action);
    }
